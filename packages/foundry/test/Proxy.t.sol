...
contract ProxyTest is Test {
  ...
  function setUp() public {
    ...
    proxy = new TransparentUpgradeableProxy(
      address(implementation), address(proxyAdmin), data
    );
  ...
  function testMintItem() public {
    ...
    assertEq(
      proxiedSVGNFT.ownerOf(tokenId), USER, "Minted token should belong to USER"
    );
    assertEq(
  ...
  function testRenderTokenById() public {
    ...
    uint256 tokenId = proxiedSVGNFT.mintItem{ value: 0.001 ether }();
    string memory render = proxiedSVGNFT.renderTokenById(tokenId);
  ...
  function testInitialTokenId() public view {
    assertEq(proxiedSVGNFT.tokenIds(), 1, "Initial tokenId should equal 1");
  }
  ...
  function testPriceIncrease() public {
    ...
    uint256 newPrice = proxiedSVGNFT.price();
    vm.stopPrank();
  ...
  function testMintLimit() public {
    uint256 currentPrice = proxiedSVGNFT.price();
    vm.startPrank(USER);
    vm.deal(USER, 10 ether);

    for (uint256 i = 1; i < 11; i++) {
      proxiedSVGNFT.mintItem{ value: currentPrice }();
      currentPrice = proxiedSVGNFT.price();
    }
  ...
  function testUpgrade() public {
    ...
    ITransparentUpgradeableProxy(address(proxy)).upgradeToAndCall(
      address(svgNFTV2), abi.encodeWithSelector(svgNFTV2.initialize.selector)
    );

    // After upgrade, proxiedSVGNFT should still work as expected
    uint256 tokenID = proxiedSVGNFT.tokenIds();
