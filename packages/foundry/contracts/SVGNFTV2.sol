...
/**
 * @title Upgrade Implementation of a simple SVG Generator based on ERC-4883
 */
contract SVGNFTV2 is ERC721Upgradeable, OwnableUpgradeable {
  ...
  uint256 public constant limit = 37;
  uint256 public constant curve = 1002; // price increase 0.4% with each purchase
  uint256 public price; // the 1154th optimistic loogies cost 0.01 ETH, the 2306th cost 0.1ETH, the 3459th cost 1 ETH and the last ones cost 1.7 ETH

  address payable public constant recipient =
    payable(0x2D31A4315e3558109c25C78b3639d316F84A203b);
  ...
  function mintItem() public payable returns (uint256) {
    require(tokenIds < limit, "DONE MINTING");
    ...
    bytes32 predictableRandom = keccak256(
      abi.encodePacked(
        id, blockhash(block.number - 1), msg.sender, address(this)
      )
    );
    color[id] = bytes2(predictableRandom[0])
      | (bytes2(predictableRandom[1]) >> 8) | (bytes3(predictableRandom[2]) >> 16);
    chubbiness[id] = 35 + ((55 * uint256(uint8(predictableRandom[3]))) / 255);
    // small chubiness loogies have small mouth
    mouthLength[id] = 180
      + (
        (uint256(chubbiness[id] / 4) * uint256(uint8(predictableRandom[4]))) / 255
      );
    eyeSize[id] = 20 + ((20 * uint256(uint8(predictableRandom[5]))) / 255);
    eyeColor[id] = bytes2(predictableRandom[6])
      | (bytes2(predictableRandom[7]) >> 8) | (bytes3(predictableRandom[8]) >> 16);
  ...
  function renderTokenById(uint256 id) public view returns (string memory) {
    ...
    string memory render = string(
      abi.encodePacked(
        ...
        "</g>" '<g class="mouth" transform="translate(',
