...
/**
 * @title Implementation of a simple SVG Generator based on ERC-4883
 */
contract SVGNFT is ERC721Upgradeable, OwnableUpgradeable {
  ...
  function mintItem() public payable returns (uint256) {
    ...
    assembly {
      ...
      if gt(sload(tokenIds.slot), limit) {
        // Store the error message
        mstore(0x00, 0xa0d36543) // SVGNFT__DONEMINTING() selector
        revert(0x00, 0x04)
      }
      ...
      id := sload(tokenIds.slot)
      sstore(tokenIds.slot, add(id, 1))
      ...
      // Set chubbiness
      let chubbinessValue :=
        add(35, div(mul(55, byte(3, predictableRandom)), 255))
      sstore(add(chubbiness.slot, id), chubbinessValue)

      // Set mouthLength
      let mouthLengthValue :=
        add(
          180, div(mul(div(chubbinessValue, 4), byte(4, predictableRandom)), 255)
        )
